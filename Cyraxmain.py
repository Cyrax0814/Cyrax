...
from Cyrax import Cyraxmain


__TELEGRAM_USERNAME__ = "𝙎𝙪𝙥𝙚𝙧𝙞𝙤𝙧𝙨99"
__CHANNEL_USERNAME__ = "𝘾𝙔𝙍𝘼𝙓 𝙏𝙀𝙍𝙈𝙐𝙓"
__GROUP_USERNAME__   = "𝘾𝙔𝙍𝘼𝙓 𝙏𝙀𝙍𝙈𝙐𝙓"
__CHANNEL_MENU__ = "𝘾𝙔𝙍𝘼𝙓 𝙈𝙀𝙉𝙐"
...
def banner(console):
    ...
    brand_name = figlet_format('CYRAX TOOL', font='speed')
    colors = [
        "rgb(255,0,0)", "rgb(253,249,249)", "rgb(253,249,249)", "rgb(253,249,249)", "rgb(253,249,249)", 
        "rgb(255,0,0)", "rgb(255,0,0)", "rgb(255,0,0)", "rgb(253,249,249)", "rgb(255,0,0)",
        "rgb(255,0,0)"
    ]
    colorful_text = gradient_text(brand_name, colors)
    console.print(colorful_text)
    print(Colorate.Horizontal(Colors.white_to_red,'==============================================================='))
    print(Colorate.Horizontal(Colors.white_to_red, '\t         𝐏𝐋𝐄𝐀𝐒𝐄 𝐋𝐎𝐆𝐎𝐔𝐓 𝐅𝐑𝐎𝐌 𝐂𝐏𝐌 𝐁𝐄𝐅𝐎𝐑𝐄 𝐔𝐒𝐈𝐍𝐆 𝐓𝐇𝐈𝐒 𝐓𝐎𝐎𝐋'))
    print(Colorate.Horizontal(Colors.white_to_red, '    𝐒𝐇𝐀𝐑𝐈𝐍𝐆 𝐓𝐇𝐄 𝐀𝐂𝐂𝐄𝐒𝐒 𝐊𝐄𝐘 𝐈𝐒 𝐍𝐎𝐓 𝐀𝐋𝐋𝐎𝐖𝐄𝐃 𝐀𝐍𝐃 𝐖𝐈𝐋𝐋 𝐁𝐄 𝐁𝐋𝐎𝐂𝐊𝐄𝐃')) 
    print(Colorate.Horizontal(Colors.white_to_red, f'      𝐂𝐎𝐍𝐓𝐀𝐂𝐓 𝐌𝐄 𝐓𝐎 𝐁𝐔𝐘 𝐔𝐍𝐋𝐈𝐌𝐈𝐓𝐄𝐃 𝐁𝐀𝐋𝐀𝐍𝐂𝐄 𝐓𝐆 @{__CHANNEL_USERNAME__}')) 
    print(Colorate.Horizontal(Colors.white_to_red, f' ‌           𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦: @{__CHANNEL_USERNAME__} 𝐎𝐫 @{__TELEGRAM_USERNAME__}'))
    print(Colorate.Horizontal(Colors.white_to_red, '==============================================================='))
...
def load_player_data(cpm):
    ...
    if response.get('ok'):
        ...
        if 'floats' in data and 'localID' in data and 'money' in data and 'coin' in data:        
            ...
            print(Colorate.Horizontal(Colors.white_to_red, f'==========[ {__CHANNEL_USERNAME__} ]=========='))
            
            print(Colorate.Horizontal(Colors.white_to_red, f'Name   : {(data.get("Name") if "Name" in data else "UNDEFINED")}.'))
                
            print(Colorate.Horizontal(Colors.white_to_red, f'LocalID: {data.get("localID")}.'))
            
            print(Colorate.Horizontal(Colors.white_to_red, f'Money  : {data.get("money")}.'))
            
            print(Colorate.Horizontal(Colors.white_to_red, f'Coins  : {data.get("coin")}.'))
            
            friends_count = len(data.get("FriendsID", []))
            print(Colorate.Horizontal(Colors.white_to_red, f'Friends: {friends_count}.'))
            car_data = data.get("carIDnStatus", {}).get("carGeneratedIDs", [])
            unique_car_data = set(car_data)
            car_count = len(unique_car_data)
            print(Colorate.Horizontal(Colors.white_to_red, f'Cars   : {car_count}.'))
            
        else:
            print(Colorate.Horizontal(Colors.white_to_red, '! ERROR: new accounts most be signed-in to the game at least once !.'))
            exit(1)
    else:
        print(Colorate.Horizontal(Colors.white_to_red, '! ERROR: seems like your login is not properly set !.'))
        exit(1)
...
def load_key_data(cpm):
    ...
    print(Colorate.Horizontal(Colors.white_to_red, f'Access Key : {data.get("access_key")}.'))
    
    print(Colorate.Horizontal(Colors.white_to_red, f'Telegram ID: {data.get("telegram_id")}.'))
    
    print(Colorate.Horizontal(Colors.white_to_red, f'Balance    : {(data.get("coins") if not data.get("is_unlimited") else " Unlimited")}.'))
...
def load_client_details():
    ...
    data = response.json()
    print(Colorate.Horizontal(Colors.white_to_red, f'==========[ {__CHANNEL_USERNAME__} ]=========='))
    print(Colorate.Horizontal(Colors.white_to_red, f'Ip Address : {data.get("query")}.'))
    print(Colorate.Horizontal(Colors.white_to_red, f'Location   : {data.get("city")} {data.get("regionName")} {data.get("countryCode")}.'))
    print(Colorate.Horizontal(Colors.white_to_red, f'Country    : {data.get("country")} {data.get("zip")}.'))
    print(Colorate.Horizontal(Colors.white_to_red, f'==========[ {__CHANNEL_MENU__} ]=========='))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        cpm = Cyraxmain(acc_access_key)
        login_response = cpm.login(acc_email, acc_password)
        if login_response != 0:
            if login_response == 100:
                print(Colorate.Horizontal(Colors.white_to_red, 'ACCOUNT NOT FOUND.'))
                sleep(2)
                continue
                ...
                print(Colorate.Horizontal(Colors.white_to_red, 'INVALID ACCESS KEY.'))
                sleep(2)
                continue
            else:
                print(Colorate.Horizontal(Colors.white_to_red, 'TRY AGAIN.'))
                print(Colorate.Horizontal(Colors.white_to_red, '! Note: make sure you filled out the fields !.'))
                sleep(2)
                continue
        else:
            print(Colorate.Horizontal(Colors.white_to_red, 'SUCCESSFUL.'))
            sleep(2)
        while True:
            banner(console)
            ...
            print(Colorate.Horizontal(Colors.white_to_red, '{01}: Increase Money           1.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{02}: Increase Coins           4.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{03}: King Rank                8.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{04}: Change ID                4.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{05}: Change Name              100'))
            print(Colorate.Horizontal(Colors.white_to_red, '{06}: Change Name (Rainbow)    100'))
            print(Colorate.Horizontal(Colors.white_to_red, '{07}: Number Plates            2.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{08}: Account Delete           FREE'))
            print(Colorate.Horizontal(Colors.white_to_red, '{09}: Account Register         FREE'))
            print(Colorate.Horizontal(Colors.white_to_red, '{10}: Delete Friends           500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{11}: Unlock Paid Cars         5.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{12}: Unlock all Cars          6.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{13}: Unlock all Cars Siren    3.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{14}: Unlock w16 Engine        4.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{15}: Unlock All Horns         3.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{16}: Unlock Disable Damage    3.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{17}: Unlock Unlimited Fuel    3.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{18}: Unlock House 3           4.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{19}: Unlock Smoke             4.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{20}: Unlock Wheels            4.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{21}: Unlock Animations        2.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{22}: Unlock Equipaments M     3.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{23}: Unlock Equipaments F     3.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{24}: Change Race Wins         1.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{25}: Change Race Loses        1.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{26}: Clone Account            7.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{27}: Custom HP                2.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{28}: Custom Angle             1.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{29}: Custom Tire burner       1.500'))
            print(Colorate.Horizontal(Colors.white_to_red, '{30}: Custom Car Millage       2.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{31}: Custom Car Brake         2.000'))
            print(Colorate.Horizontal(Colors.white_to_red, '{32}: Login Another Account    FREE'))
            print(Colorate.Horizontal(Colors.white_to_red, '{0} : Exit From CYRAX Menu     FREE'))
            ...
            if service == 0: # Exit
                print(Colorate.Horizontal(Colors.white_to_red, f'Thank You for using our tool, please join our telegram channel: @{__CHANNEL_USERNAME__}.'))
                
            elif service == 1: # Increase Money
                print(Colorate.Horizontal(Colors.white_to_red, '[?] Insert how much money do you want.'))
                ...
                if amount > 0 and amount <= 500000000000000000000000000000000:
                    if cpm.set_player_money(amount):
                        ...
                        print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
                        print(Colorate.Horizontal(Colors.white_to_red, 'Please try again.'))
                        sleep(2)
                        continue
                else:
                    print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
                    print(Colorate.Horizontal(Colors.white_to_red, 'Please use valid values.'))
                    sleep(2)
                    continue
            elif service == 2: # Increase Coins
                print(Colorate.Horizontal(Colors.white_to_red, '[?] Insert how much coins do you want.'))
                amount = IntPrompt.ask("[?] Amount")
                console.print("[%] Saving your data: ", end=None)
                ...
                if cpm.set_player_rank():
                    ...
                    print(Colorate.Horizontal(Colors.white_to_red, '======================================'))
                    answ = Prompt.ask("[?] Do You want to Exit ?", choices=["y", "n"], default="n")
                    if answ == "y": print(Colorate.Horizontal(Colors.white_to_red, f'Thank You for using our tool, please join our telegram channe: @{__CHANNEL_USERNAME__}.'))
                    else: continue
                else:
                    print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
                    print(Colorate.Horizontal(Colors.white_to_red, 'Please try again.'))
                ...
                print(Colorate.Horizontal(Colors.white_to_red, '[?] Enter your new ID.'))
                ...
                if len(new_id) >= 0 and len(new_id) <=  and (' ' in new_id) == False:
                    ...
                    print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
                    print(Colorate.Horizontal(Colors.white_to_red, 'Please use valid ID.'))
                    sleep(2)
                    continue
            elif service == 5: # Change Name
                print(Colorate.Horizontal(Colors.white_to_red, '[?] Enter your new Name.'))
                ...
                print(Colorate.Horizontal(Colors.white_to_red, '[?] Enter your new Rainbow Name.'))
                new_name = Prompt.ask("[?] Name")
                console.print("[%] Saving your data: ", end=None)
                if len(new_name) >= 0 and len(new_name) <= 999999999:
                ...
                print(Colorate.Horizontal(Colors.white_to_red, '[!] After deleting your account there is no going back !!.'))
                answ = Prompt.ask("[?] Do You want to Delete this Account ?!", choices=["y", "n"], default="n")
                if answ == "y":
                    cpm.delete()
                    print(Colorate.Horizontal(Colors.white_to_red, 'SUCCESSFUL'))
                    print(Colorate.Horizontal(Colors.white_to_red, '======================================'))
                    print(Colorate.Horizontal(Colors.white_to_red, f'Thank You for using our tool, please join our telegram channe: @{__CHANNEL_USERNAME__}.'))
                else: continue
            elif service == 9: # Account Register
                print(Colorate.Horizontal(Colors.white_to_red, '[!] Registring new Account.'))
                ...
                if status == 0:
                    print(Colorate.Horizontal(Colors.white_to_red, 'SUCCESSFUL'))
                    print(Colorate.Horizontal(Colors.white_to_red, '======================================'))
                    print(Colorate.Horizontal(Colors.white_to_red, f'INFO: In order to tweak this account with CPMElsedev.'))
                    print(Colorate.Horizontal(Colors.white_to_red, 'you most sign-in to the game using this account.'))
                    sleep(2)
                    continue
                elif status == 105:
                    print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
                    print(Colorate.Horizontal(Colors.white_to_red, 'This email is already exists !.'))
                    sleep(2)
                    continue
                else:
                    print(Colorate.Horizontal(Colors.white_to_red, 'FAILED.'))
